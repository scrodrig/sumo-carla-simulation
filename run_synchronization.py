...
class SimulationSynchronization(object):
    ...
    def tick(self):
        ...
        for sumo_actor_id in self.sumo2carla_ids:
            ...
            ################## mine
            # print(carla_actor_id, carla_transform, sep="---")
            # with open('./testCarla2Sumo.csv', 'a', newline='') as file:
            #     writer = csv.writer(file)
            #     writer.writerow(["SNo", "Name", "Subject", "time"])
            # print(sumo_actor.extent.x, sep="---")
            # print(sumo_actor, carla_transform, sep="---")
            # ["CarlaId", "SumoId", "Model", "Carla X", "Carla Y", "Carla Z", "time"]
            with open('./testCarla2Sumo.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([carla_actor_id, 
                                 sumo_actor_id, 
                                 sumo_actor.type_id,
                                 "bot", 
                                 ...
                                 sumo_actor.transform.location.x, 
                                 sumo_actor.transform.location.y, 
                                 sumo_actor.transform.location.z,
                                 sumo_actor.extent.x, 
                                 sumo_actor.extent.y, 
                                 sumo_actor.extent.z,
                                time.time()])
        ...
        for carla_actor_id in self.carla2sumo_ids:
            ...
            carla_blueprint = BridgeHelper.get_carla_blueprint(sumo_actor, self.sync_vehicle_color)
            carla_role = carla_actor.attributes.get("role_name")
            carla_transform = carla_actor.get_transform()
             ################## mine
            # print("Here in the other thick")
            # print(carla_actor, sumo_actor_id, carla_role, sumo_actor ,carla_blueprint, sep="<----->")
            with open('./testCarla2Sumo.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([carla_actor_id, 
                                 sumo_actor_id, 
                                 ...
                                 carla_transform.location.y, 
                                 carla_transform.location.z,
                                 sumo_transform.location.x, 
                                 sumo_transform.location.y, 
                                 sumo_transform.location.z,
                                 sumo_actor.extent.x, 
                                 sumo_actor.extent.y, 
                                 sumo_actor.extent.z,
                                time.time()])
...
def synchronization_loop(args):
    ...
    with open('./testCarla2Sumo.csv', 'a', newline='') as file:
            ...
            writer.writerow(["CarlaId", 
                             ...
                             "SUMO Vector Z", 
                             "time"])
    # with open('./testSumo2Carla.csv', 'a', newline='') as file:
    #         writer = csv.writer(file)
    #         writer.writerow(["CarlaId", 
    #                          "SumoId", 
    #                          "Model", 
